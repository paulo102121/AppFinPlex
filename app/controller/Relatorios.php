    ...
    class Relatorios extends Controller
    {
        ...
        {   
            ...
            try {
                ...
                $sql ="SELECT MONTH(desp_data) as mes,YEAR(desp_data) as ano FROM receitas WHERE id_usuario = ".Usuario::get('id')." GROUP BY ano,mes";
                ...
                if(!empty($_POST) && isset($_POST['tipos']) && isset($_POST['mes-ano']))
                {
                    ...
                    switch ($_POST['tipos']) {
                        case 1:
                            $sql = "SELECT SUM(d.valor) as total,c.nome as categoria FROM despesas as d INNER JOIN categorias as c ON d.id_categoria = c.id WHERE d.id_usuario = ".Usuario::get('id')." AND MONTH(d.desp_data) = {$mesAno[0]} AND YEAR(d.desp_data) = {$mesAno[1]} GROUP BY categoria";
                            break;
                        case 2:
                            $sql = "SELECT SUM(d.valor) as total,c.nome as categoria FROM receitas as d INNER JOIN categorias as c ON d.id_categoria = c.id WHERE d.id_usuario = ".Usuario::get('id')." AND MONTH(d.desp_data) = {$mesAno[0]} AND YEAR(d.desp_data) = {$mesAno[1]} GROUP BY categoria";
                            break;
                        case 3:
                            break;
                    }
                    ...
                    $sql = "SELECT SUM(d.valor) as total,c.nome as categoria FROM despesas as d INNER JOIN categorias as c ON d.id_categoria = c.id WHERE d.id_usuario = ".Usuario::get('id')." AND MONTH(d.desp_data) = MONTH(CURRENT_DATE()) AND YEAR(d.desp_data) = YEAR(CURRENT_DATE()) GROUP BY categoria";
                                    /*Soma total das Receitas Inseridas */
                    $criteria2 = new Criteria;
                    $criteria2->add('id_usuario','=',Usuario::get('id'));
