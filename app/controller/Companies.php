...
/*
              ...
              //  echo $sql;
                ...
                */
                 $db = new DBmanager();
                    $empresas = $db->from('empresas')
              //  ->join('usuario', array('empresas.id' => 'usuario.empresa_id')) //, 'usuario.id' => Usuario::get('id')))
               // ->select("empresas.*, usuario.nome as username")
               ->select()
                ->execute();
                // while ($obj = $empresa->fetch_object()) {$empresas = $obj;}
                 /*
                if($obj==null){
                    $user = $db->from('usuario')
