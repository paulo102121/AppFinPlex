    ...
    class Home extends Controller
    {
        public function index()
        {
            ...
            try {
                ...
                /*************************  Retorna total despesas por Categoria *************************/
                $resp = new Repository('app\model\class\Conta');

                $sql = "SELECT c.nome as categoria, SUM(d.valor) as total FROM despesas as d INNER JOIN categorias as c ON d.id_categoria = c.id WHERE d.id_usuario = ".Usuario::get('id')." AND MONTH(d.desp_data) = MONTH(CURRENT_DATE()) AND YEAR(d.desp_data) = YEAR(CURRENT_DATE()) GROUP BY categoria;";
                ...
                $sql = "SELECT c.nome as categoria, SUM(d.valor) as total FROM receitas as d INNER JOIN categorias as c ON d.id_categoria = c.id WHERE d.id_usuario = ".Usuario::get('id')." AND MONTH(d.desp_data) = MONTH(CURRENT_DATE()) AND YEAR(d.desp_data) = YEAR(CURRENT_DATE()) GROUP BY categoria;";
                ...
                /******* Retorna total de despesas de cada mes do ano *******/
                $despesaMes = new Repository('StdClass');

                $sqlDepesaMes = "SELECT MONTH(desp_data) as mes,SUM(valor) as total FROM despesas WHERE id_usuario = ".Usuario::get('id')." AND YEAR(desp_data) = YEAR(CURRENT_DATE())  GROUP BY mes";
                ...
                /******* Retorna total de receitas de cada mes do ano *******/
                $receitaMes = new Repository('StdClass');

                $sqlReceitaMes = "SELECT MONTH(desp_data) as mes,SUM(valor) as total FROM receitas WHERE id_usuario = ".Usuario::get('id')." AND YEAR(desp_data) = YEAR(CURRENT_DATE()) GROUP BY mes";
